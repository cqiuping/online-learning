				...
				<section class="content-header">
					<h1>
						<%
							...
							{
						%>
						<span class="text-green">成绩合格，快进行下面的学习吧</span>
						<%
							}
							else{
						...
						%>
						<span class="text-red">成绩不合格，要加强复习啊</span>
						<%} %>
						<small>点击对应标签可以复习要看的课程哟~</small>
					</h1>
					<ol class="breadcrumb">
					...
					<form name="form" id="formid">
						...
						if(questions!=null)
						{
							for(int i=0;i<questions.size();i++)
							{
								...
								if((check.charAt(i)+"").equals("0"))
								{
					%>
						<dl class="swiper-slide">
							<dt><%=i+1 %>、<%=qcontent[0] %><span class="glyphicon glyphicon-remove text-red"></span> </dt>
							<%
								}
							else
							{
								%>
								<dl class="swiper-slide">
							<dt><%=i+1 %>、<%=qcontent[0] %><span class="glyphicon glyphicon-ok text-green"></span> </dt>
								<% 
								}
						...
						<dd><font color="#888888">标签：</font>
						<%
							String[] tag=qdao.splitTag(q.getTag());
							for(int k=0;k<tag.length;k++)
							{
								Integer vid=qdao.getVidByTag(tag[k], cid);
								if(vid!=null)
								{
						%>
						<a href="../main/video.jsp?vid=<%=vid %>&look=1" target="_blank"><span class="text-yellow"><%=tag[k] %></span></a>
						<%
								}
						} 
						%>
