<%@page import="com.po.Type.UserQuestionPK"%>
						...
						<ul class="nav navbar-nav">
							...
							<li class="dropdown user user-menu">
								...
								data-toggle="dropdown"> <!-- The user image in the navbar-->
									...
									<span class="hidden-xs"> <%out.print(user!=null?user.getUsername():"");%>
								</span>
							</a>
								<ul class="dropdown-menu">
				...
				<section class="content">
					...
					<form name="form" id="formid">
						<dl class="swiper-slide">
							<dd>
								对于这门课，您想要达到的学习程度:&nbsp;&nbsp;&nbsp;
								<label><input name="degree"  type="radio" value="known" /><span class="text-red">了解</span> </label> 
								&nbsp;&nbsp;&nbsp;<label><input name="degree"  type="radio" value="familiar" /><span class="text-red">熟悉</span> </label> 
								&nbsp;&nbsp;&nbsp;<label><input name="degree"   type="radio" value="master" /><span class="text-red">精通</span> </label> 
							</dd>
						</dl>
						<%
						...
						if(questions!=null)
						{
							...
							{
								q=questions.get(i);
								qcontent=qdao.splitQuestionContent(q.getQcontent());
								//从user_question中查找是否有对应的题目 
								UserQuestionPK pk=new UserQuestionPK();
								pk.setQid(q.getQid());
								pk.setUid(user.getUid());
								UserQuestionDao uqDao=new UserQuestionDao();
								UserQuestion userQuestion=uqDao.getUserQuestionByPK(pk);
								
					%>
						<dl class="swiper-slide">
							<dt><%=i+1 %>、<%=qcontent[0] %></dt>
							<%
								for(int j=1;j<qcontent.length;j++)
								{
									if(userQuestion!=null)
									{
										if(userQuestion.getTOrF()==1.0)//说明答案是对的
										{
											if(j==2) //要选择2
											{
							%>
							<dd>
								<input type="radio" name="que_<%=q.getQid() %>" value=<%=j %> checked="checked" /><%=qcontent[j] %>
							</dd>
							<%
											}
											else
							...
							%>
							<dd>
								<input type="radio" name="que_<%=q.getQid() %>" value=<%=j %>/><%=qcontent[j] %>
							</dd>
						<%
											}
										}
						...
						%>
							<dd>
								<input type="radio" name="que_<%=q.getQid() %>" value=<%=j %>
									checked="checked" /><%=qcontent[j] %>
							</dd>
						<%
											}
											else
					...
					<input type="button" name="tsub"  value="提交"
					...
					<script language="javascript">
						...
						function submitit() {
							...
							/*var sic = $("input[name='sic']:checked").val();
							var chi = $("input[name='chi']:checked").val();
							var param = sic + "," + chi;
							form
									.setAttribute("action",
											"../servlet/QuestionServlet?param="
													+ param);
							...
							for(var i=1;i<=len;i++)
							{
								...
								else val+=","+$("input[name="+n+"]:checked").val();
							}
							form
							.setAttribute("action",
									"../servlet/QuestionServlet?answer="
											+ val+"&cid="+<%=cid%>+"&degree="+degree);
